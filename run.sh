...
if [ -n "$keep" ] && [ -s thread_log/run.log ]; then
    ...
    if [ -z "$prev" ]; then
        echo "run.sh: existing log carries no run marker; appending without a config check" >&2
    elif [ "$prev" != "$config_id" ] && [ -z "$force" ]; then
        echo "run.sh: src/main.c changed since the log's last run ($prev -> $config_id)." >&2
...
make build
time ./src/main.out "${args[@]}" PI 2> >(tee -a thread_log/run.log >&2)
